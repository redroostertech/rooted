...
open class TokenCell<T: TokenSearchable>: Cell<Set<T>>, CLTokenInputViewDelegate, TokenCellProtocol, CellType {
  ...
  public var textForSearch: ((String, BaseCell, BaseRow) -> Void)?
    ...
    open override func update() {
        ...
        let valueIdentifiers = Set(row.value?.compactMap { $0.id_fier } ?? [])
        ...
        if Set(tokenView.allTokens.compactMap { $0.context }) != valueIdentifiers {
            ...
            row.value?.forEach({ (tokenSearchable) in
                if valuesToAdd.contains(tokenSearchable.id_fier) {
  ...
  open func disableTextField() { }
    ...
    open func tokenInputView(_ view: CLTokenInputView, didChangeText text: String?) {
      if let txtForSearch = textForSearch {
        txtForSearch("\(view.text ?? "")\(text ?? "")", self, row)
      }
    }
