...
open class TableTokenCell<T, TableViewCell: UITableViewCell>: TokenCell<T>, UITableViewDelegate, UITableViewDataSource where TableViewCell: EurekaTokenTableViewCell, TableViewCell.T == T {
    ...
    open override func showOptions() {
        if let controller = formViewController() as? FormMessagesAppViewController {
  ...
  override open func disableTextField() {
    tokenView.isUserInteractionEnabled = false
  }

  @objc open override func tokenInputView(_ aView: CLTokenInputView, didChangeText text: String?) {
