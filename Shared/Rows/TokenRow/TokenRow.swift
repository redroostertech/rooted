...
open class _TokenRow<T: TokenSearchable, Cell: BaseCell> : Row<Cell> where Cell: CellType, Cell: TokenCellProtocol, Cell.Value == Set<T> {
  ...
  // POC
  public var textForSearch: ((String, BaseCell, BaseRow) -> Void)? {
    didSet {
      // TODO: - Add extensibility here
      guard let tokenCell = cell as? TokenCell<RContact> else { return }
      tokenCell.textForSearch = self.textForSearch
    }
  }
    ...
    open func removeToken(_ tokenIdentifier: NSObject) {
        if let token = value?.filter({ $0.id_fier == tokenIdentifier }).first {
            removeToken(token)
        }
    }
    ...
    open func removeToken(_ token: T) {
        value?.remove(token)
        if let cltoken = cell.tokenView.allTokens.filter({ $0.context == token.id_fier }).first {
    ...
    open func addToken(_ tokenIdentifier: NSObject) {
        if let token = options.filter({$0.id_fier == tokenIdentifier}).first {
            addToken(token)
        }
    }
    ...
    open func addToken(_ token: T) {
        value?.insert(token)
        cell.tokenView.add(CLToken(displayText: token.displayString, context: token.id_fier))
    }
}
